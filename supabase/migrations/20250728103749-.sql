...
-- Create policies for online status (drop existing if they exist)
DROP POLICY IF EXISTS "Online status is viewable by everyone" ON public.user_online_status;
DROP POLICY IF EXISTS "Users can update their own online status" ON public.user_online_status;
DROP POLICY IF EXISTS "Users can insert their own online status" ON public.user_online_status;
...
-- Create trigger for updated_at (drop existing if it exists)
...
-- Add indexes for performance (create if not exists)
CREATE INDEX IF NOT EXISTS idx_user_online_status_user_id ON public.user_online_status(user_id);
CREATE INDEX IF NOT EXISTS idx_user_online_status_is_online ON public.user_online_status(is_online);
